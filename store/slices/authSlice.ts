...
import {
  tipoAdmin,
  roles,
} from "../../interfaces/storeInterfaces/sliceAdminInterface";
...
export const getAuth = createAsyncThunk("getDataAdmin", async () => {
  try {
    ...
    const response = await axiosInstances.get<dataInfo>(
      "administrador/get_data_session",
      {
        headers: { TOKEN: token },
      }
    );
...
export const { loggout } = authSlice.actions;
